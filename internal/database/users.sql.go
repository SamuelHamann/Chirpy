...
const deleterAllUsers = `-- name: DeleterAllUsers :many
DELETE FROM users
RETURNING id, created_at, updated_at, email, hashed_password, is_chirpy_red
`

func (q *Queries) DeleterAllUsers(ctx context.Context) ([]User, error) {
	rows, err := q.db.QueryContext(ctx, deleterAllUsers)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (q *Queries) UpdateUser(ctx context.Context, arg UpdateUserParams) (User, error) {
	row := q.db.QueryRowContext(ctx, updateUser, arg.ID, arg.Column2, arg.Column3)
	var i User
	err := row.Scan(
		&i.ID,
		&i.CreatedAt,
